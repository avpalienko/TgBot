...
go build -ldflags="-s -w -X %PKG%.GitCommit=%GIT_COMMIT% -X %PKG%.GitDate=%GIT_DATE% -X %PKG%.GitBranch=%GIT_BRANCH% -X %PKG%.BuildDate=%BUILD_DATE%" -o tgbot ./cmd/bot
