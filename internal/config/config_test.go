...
func setCleanEnv(t *testing.T) {
	...
	for _, key := range []string{
		...
		"OPENAI_MAX_RETRIES", "ALLOWED_USERS", "MAX_HISTORY", "SESSION_TTL",
		"MAX_CONCURRENCY", "REQUEST_TIMEOUT", "MAX_PROMPT_LENGTH", "LOG_LEVEL", "LOG_FORMAT",
	} {
		t.Setenv(key, "")
	}
}
...
func TestLoadMinimal(t *testing.T) {
	...
	if cfg.OpenAIMaxRetries != 3 {
		t.Fatalf("expected default OpenAIMaxRetries 3, got %d", cfg.OpenAIMaxRetries)
	}
	if cfg.MaxConcurrency != 20 {
		t.Fatalf("expected default maxConcurrency 20, got %d", cfg.MaxConcurrency)
	}
	if cfg.SessionTTL != 24*time.Hour {
		t.Fatalf("expected default sessionTTL 24h, got %v", cfg.SessionTTL)
	}
	if cfg.RequestTimeout != 60*time.Second {
		t.Fatalf("expected default requestTimeout 60s, got %v", cfg.RequestTimeout)
	}
	if cfg.MaxPromptLength != 4000 {
		t.Fatalf("expected default MaxPromptLength 4000, got %d", cfg.MaxPromptLength)
	}
...
func TestLoadFull(t *testing.T) {
	...
	t.Setenv("REQUEST_TIMEOUT", "2m30s")
	t.Setenv("MAX_PROMPT_LENGTH", "8000")
	...
	if cfg.OpenAIMaxRetries != 5 {
		t.Fatalf("expected OpenAIMaxRetries 5, got %d", cfg.OpenAIMaxRetries)
	}
	...
	if cfg.SessionTTL != 90*time.Minute {
		t.Fatalf("expected sessionTTL 1h30m, got %v", cfg.SessionTTL)
	}
	if cfg.RequestTimeout != 150*time.Second {
		t.Fatalf("expected requestTimeout 2m30s, got %v", cfg.RequestTimeout)
	}
	if cfg.MaxPromptLength != 8000 {
		t.Fatalf("expected MaxPromptLength 8000, got %d", cfg.MaxPromptLength)
	}
...
func TestLoadInvalidMaxRetries(t *testing.T) {
	...
	_, err := Load()
	if err == nil {
		t.Fatalf("expected error for non-numeric OPENAI_MAX_RETRIES")
	}
}
...
func TestLoadInvalidMaxPromptLength(t *testing.T) {
	setCleanEnv(t)
	t.Setenv("TELEGRAM_BOT_TOKEN", "tok")
	t.Setenv("OPENAI_API_KEY", "key")
	t.Setenv("MAX_PROMPT_LENGTH", "not-a-number")

	_, err := Load()
	if err == nil {
		t.Fatalf("expected error for non-numeric MAX_PROMPT_LENGTH")
	}
}
...
func TestLoadCustomSessionTTL(t *testing.T) {
	setCleanEnv(t)
	t.Setenv("TELEGRAM_BOT_TOKEN", "tok")
	t.Setenv("OPENAI_API_KEY", "key")
	...
	if cfg.SessionTTL != 720*time.Hour {
		t.Fatalf("expected sessionTTL 720h, got %v", cfg.SessionTTL)
	}
}
